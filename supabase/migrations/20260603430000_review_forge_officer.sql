-- ============================================================
-- Officers couldn't approve their own unit's forge requests (GH #7): review_forge
-- required review_skillcard (badges) / manage_resources (resources), which a
-- chapter officer may not hold. Let the officer who stewards the member/holder
-- approve those too. Capability holders (admins) still approve anything.
-- (Same body as 300000 + broadened badge/resource auth.)
-- ============================================================
...
begin
  ...
  if r.target_type = 'badge' then
    if not (has_capability('review_skillcard') or has_capability('manage_members')
            or is_unit_officer_of((r.payload->>'member_id')::uuid)) then
      raise exception 'requires review_skillcard or being the member''s unit officer'; end if;
  elsif r.target_type = 'resource' then
    if not (has_capability('manage_resources')
            or is_unit_officer_of((r.payload->>'holder_member_id')::uuid)) then
      raise exception 'requires manage_resources or being the holder''s unit officer'; end if;
