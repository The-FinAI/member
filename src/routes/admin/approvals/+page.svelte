<script lang="ts">
  ...
  // Unit membership applications — the one approval queue not covered by the
  // forge queue. Everything else (badges, member cards, needs, resources,
  // over-capacity commitments, settlements) lives on /admin/forge-queue now.
  // Admins (manage_members) see every pending application; an officer sees the
  // applications to the chapters / working groups they run.
  type UnitApp = {
  ...
  const canReview = $derived(isAdmin || isOfficer);
...
<div class="stack">
  <header>
    <h1>{$t('Unit applications')}</h1>
    <p class="muted sub">{$t('Members applying to join a chapter or working group. Everything else awaiting review is on the')} <a href="/admin/forge-queue">{$t('forge queue')}</a>.</p>
  </header>
        ...
        <div class="app">
          <div class="app-main">
            ...
            <span class="app-sub">
              ...
              {$t('applied to')} <strong>{a.org_unit?.name ?? '—'}</strong>
              {#if a.applied_on} · {when(a.applied_on)}{/if}
            </span>
          </div>
          <div class="app-act">
...
<style>
  ...
  .app {
    ...
    padding: .7rem .9rem; border: 1px solid var(--border); border-radius: 11px; background: var(--card);
  }
  ...
  .btn {
    padding: .4rem .8rem; border-radius: 8px; border: 1px solid transparent;
