<script lang="ts">
  ...
  // Unified Forge approval queue — ONE entry point over forge_request, plus the
  // over-capacity work_commitment queue. Dispatches to review_forge / review_capacity.
  type Req = {
  ...
  let filter = $state<'all' | 'badge' | 'resource' | 'need' | 'project_done'>('all');

  const TYPE_LABEL: Record<string, string> = {
    member_card: 'Member card', badge: 'Badge', resource: 'Resource',
    need: 'Need', claim: 'Claim', project_done: 'Completion'
  };
  ...
  async function load() {
    ...
    loading = true; msg = '';
    const [{ data: rq }, { data: cap }, { data: m }, { data: sk }, { data: pr }, { data: rt }] = await Promise.all([
...
<div class="fq">
  <header class="fq-head">
    ...
    <div class="fq-filters">
      {#each ['all', 'badge', 'resource', 'need', 'project_done'] as f}
