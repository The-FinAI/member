<script lang="ts">
  ...
  // Shared body for the project page. Used by the /projects/[id] route and by
  // the project quick-view drawer in Market, so the drawer mirrors the full
  ...
  let lastId = '';
  $effect(() => { if (id && id !== lastId) { lastId = id; load(); } });
